#include "VulkanRenderer/Render/RenderPassManager.hpp"

void RenderPassManager::Initialize(VkDevice device, VkFormat swapchainImageFormat) {
    device_ = device;
    ...
    // Finally, assemble the render pass info
...
void RenderPassManager::BeginRenderPass(VkCommandBuffer cmdBuffer, VkFramebuffer framebuffer, VkExtent2D extent, const VkClearValue* clearValues, uint32_t clearValueCount) {
    if (renderPass_ == VK_NULL_HANDLE) {
...
void RenderPassManager::EndRenderPass(VkCommandBuffer cmdBuffer) {
